  ...
  return (
    <section className="w-full my-10 mx-auto ">
      <div className="text-center mb-6 space-y-2">
        <h3 className="text-2xl font-bold ">✨ Kategori Pilihan Terbaik</h3>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6 gap-2">
        {sortedCategories.map((category: Category) => (
          <button
            key={category.id}
            className={`${
              category.id === stateCategory ? "bg-primary" : "bg-secondary"
            } flex items-center gap-2  p-2 rounded-full border hover:bg-primary  transition-colors text-sm font-medium `}
            onClick={() => {
